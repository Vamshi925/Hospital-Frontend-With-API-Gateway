...
const fetchNotifications = async () => {
    ...
    if (userRole === 'ROLE_PATIENT' && patientId) {
      ...
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/login-service/api/patients/notifications/${patientId}`, {
          headers: {
    ...
    if (userRole === 'ROLE_DOCTOR' && doctorId) {
      ...
      try {
        const token = getJwtToken();
        const response = await fetch(`${API_BASE_URL}/doctor-service/api/doctors/notifications/${doctorId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
