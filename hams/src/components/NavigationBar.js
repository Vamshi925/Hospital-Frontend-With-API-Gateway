...
function NavigationBar() {
  ...
  const fetchNotifications = async () => {
    ...
    if (userRole === 'ROLE_PATIENT' && patientId) {
      ...
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}/login-service/api/patients/notifications/${patientId}`,
          {
            headers: {
    ...
    if (userRole === 'ROLE_DOCTOR' && doctorId) {
      ...
      try {
        const token = getJwtToken();
        const response = await fetch(
          `${API_BASE_URL}/doctor-service/api/doctors/notifications/${doctorId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const fetchDoctorDetails = async () => {
    if (userRole === 'ROLE_DOCTOR' && doctorId) {
      ...
      try {
        const token = getJwtToken();
        const response = await fetch(`${API_BASE_URL}/doctor-service/api/doctors/${doctorId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const fetchPatientDetails = async () => {
    if (userRole === 'ROLE_PATIENT' && patientId) {
      ...
      try {
        const token = getJwtToken();
        const response = await fetch(`${API_BASE_URL}/login-service/api/patients/${patientId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const getLinks = () => {
    ...
    if (userRole === 'ROLE_DOCTOR') {
      return (
        <>
          <Link to={`/doctor/${userId}/dashboard`} className="nav-link">
            Dashboard
          </Link>
          <Link to={`/doctor/${userId}/appointments`} className="nav-link">
            Appointments
          </Link>
          <Link to={`/doctor/${userId}/schedule`} className="nav-link">
            Schedule
          </Link>
          <Link to={`/doctor/${userId}/prescriptions`} className="nav-link">
            Prescriptions
          </Link>
          <Link to={`/doctor/${userId}/past-appointments`} className="nav-link">
            Past Appointments
          </Link>
        </>
      );
      ...
      return (
        <>
          <Link to={`/patient-dashboard/${userId}`} className="nav-link">
            Dashboard
          </Link>
          <div
          ...
          >
            ...
            {showAppointmentsDropdown && (
              <div className="appointments-dropdown">
                <Link to="/appointments" className="dropdown-link">
                  Current Appointments
                </Link>
                <Link to="/pastappointments" className="dropdown-link">
                  Past Appointments
                </Link>
              </div>
            )}
      ...
      <div className="nav-links">
        ...
        {isLoggedIn && (userRole === 'ROLE_DOCTOR' || userRole === 'ROLE_PATIENT') && (
          <>
              ...
              <div className="icon-container-nav profile-nav">
                ...
                {showDoctorDetailsPopup && doctorDetails && (
                  <div className="details-popup-nav" ref={detailsPopupRef}>
                    ...
                    <p>
                      <span className="detail-label">Name:</span> {doctorDetails.name}
                    </p>
                    <p>
                      <span className="detail-label">Specialization Name:</span>{' '}
                      {doctorDetails.specializationName}
                    </p>
                    <p>
                      <span className="detail-label">Gender:</span> {doctorDetails.gender}
                    </p>
                    <p>
            ...
            {showPatientDetails && (
              <div
                className="profile-details-expanded"
                ref={detailsPopupRef}
              ...
              >
                {loadingPatient ? (
                  ...
                  <>
                    <p>
                      <span className="detail-label">Name:</span> {patientInfo.name}
                    </p>
                    {/* <p><span className="detail-label">ID:</span> {patientInfo.id}</p> */}
                    <p>
                      <span className="detail-label">Gender:</span> {patientInfo.gender}
                    </p>
                    <p>
                      <span className="detail-label">Weight:</span> {patientInfo.weightInKg}
                    </p>
                    <p>
                      <span className="detail-label">Height:</span> {patientInfo.heightInCm}
                    </p>
                    <p>
                      <span className="detail-label">Phone:</span> {patientInfo.phoneNumber}
                    </p>
                    <p>
                      <span className="detail-label">Age:</span> {patientInfo.age}
                    </p>
