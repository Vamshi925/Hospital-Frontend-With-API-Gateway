...
const AuthPage = ({ setIsLoggedIn, setUserRole }) => {
  ...
  const handleSignIn = async (e) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/login-service/api/users/login`, {
        method: 'POST',
        headers: {
        ...
        if (response.status === 401) {
          try {
            ...
            if (contentType && contentType.includes('application/json')) {
              ...
              setSignInError(
                errorData.message || 'Bad credentials. Please check your email and password.'
              );
            } else {
  ...
  const handleSignUpSubmit = async (e) => {
    ...
    if (Object.keys(formErrors).length === 0) {
      ...
      setSignUpError('');
      try {
        const response = await fetch(`${API_BASE_URL}/login-service/api/users/register/patient`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        if (response.ok) {
          ...
          if (errorData && errorData.message === 'Validation failed' && errorData.data) {
            ...
            errorData &&
            errorData.message === 'Duplicate Entry : User with this email already exists'
          ) {
          ...
          {(isSignIn || role === 'ROLE_DOCTOR') && (
            <form onSubmit={handleSignIn} className="auth-form">
              ...
              <button type="submit" className="auth-button" disabled={isSignInLoading}>
                {isSignInLoading
                  ? 'Signing in...'
                  : `Sign In as ${role === 'ROLE_PATIENT' ? 'Patient' : 'Doctor'}`}
              </button>
            ...
            <form onSubmit={handleSignUpSubmit} className="auth-form" data-mode="signup">
              ...
              <div className="form-group">
                ...
                {signUpErrors.phoneNumber && (
                  <div className="error-message">{signUpErrors.phoneNumber}</div>
                )}
              </div>
              <div className="form-group">
                ...
                {signUpErrors.dateOfBirth && (
                  <div className="error-message">{signUpErrors.dateOfBirth}</div>
                )}
              </div>
              <div className="form-group">
                <label>Password</label>
              ...
              <div className="form-group">
                ...
                {signUpErrors.retypePassword && (
                  <div className="error-message">{signUpErrors.retypePassword}</div>
                )}
              </div>
