...
const DynamicDoctorAppointment = () => {
  ...
  useEffect(() => {
    if (selectedSpecializationId) {
      ...
      axios
        .get(
          `${API_BASE_URL}/doctor-service/api/doctors/specialization/id/${selectedSpecializationId}`,
          { headers: { Authorization: `Bearer ${token}` } }
        )
        .then((response) => {
          if (response.data.success && response.data.data) setDoctors(response.data.data);
            ...
            setDoctors([]);
            setError("No Doctor's Available for the selected criteria.");
          }
        ...
        .then((response) => {
          if (response.data.success && response.data.data) {
            ...
            const formattedData = response.data.data
              .filter(
                (item) =>
                  new Date(item.availableDate) >= today &&
                  new Date(item.availableDate) <= futureDate
              )
              .map((item) => ({
            ...
            console.log('Failed to fetch doctor availability for popup.');
          }
        })
        .catch((error) => {
          setDoctorAvailabilityData([]);
  ...
  const handleBookAppointmentSubmission = async () => {
    if (selectedSpecializationId && selectedDoctorId && appointmentDate && selectedSession) {
      ...
      const selectedSpecialization = specializations.find(
        (spec) => spec.specializationId === selectedSpecializationId
      );
      ...
      if (!patientId || !token) {
        setError(
          patientId
            ? 'Token not found. Please ensure you are logged in.'
      ...
      try {
        ...
        if (response.data.success) {
          ...
          const errorMessage =
            response.data.data?.message || response.data.message || 'Failed to book appointment';
      ...
      setError(
        'Please select specialization, doctor, and a date/time slot to book an appointment.'
      );
      setBookingSuccess(false);
    }
  };
        ...
        <div className="book-appointment-card">
          ...
          <div className="form-group">
            ...
            <select
              ...
              disabled={!selectedSpecializationId || loadingDoctors || doctors.length === 0}
            >
              <option value="">
                {loadingDoctors
                  ? 'Loading Doctors...'
          ...
          <div className="form-group">
            ...
            {selectedDoctorId ? (
              <button
                ...
                onClick={() => setIsAvailabilityPopupVisible(true)}
              >
                View Availability
              </button>
            ) : (
            ...
            {selectedSession && (
              <p>
                Selected Session:{' '}
                {sessions.find((s) => s.value === selectedSession)?.label || 'No Session Selected'}
              </p>
            )}
          </div>
                ...
                <div className="availability-grid">
                  {doctorAvailabilityData.map((item) => (
                    ...
                    >
                      ...
                      {item.available && (
                        <span className="session-indicator dynamic">{item.session}</span>
                      )}
                    </div>
                  ))}
                </div>
          ...
          <button
            ...
            onClick={handleBookAppointmentSubmission}
            disabled={
              !selectedSpecializationId ||
              !selectedDoctorId ||
              !appointmentDate ||
