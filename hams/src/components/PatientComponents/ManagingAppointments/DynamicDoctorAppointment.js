...
const DynamicDoctorAppointment = () => {
    ...
    useEffect(() => {
        ...
        axios.get('${API_BASE_URL}/doctor-service/specialization/getAll', { headers: { Authorization: `Bearer ${token}` } })
            .then(response => {
    ...
    useEffect(() => {
        if (selectedSpecializationId) {
            ...
            const token = getJwtToken();
            axios.get(`${API_BASE_URL}/doctor-service/api/doctors/specialization/id/${selectedSpecializationId}`, { headers: { Authorization: `Bearer ${token}` } })
                .then(response => {
    ...
    const fetchDoctorAvailability = (doctorId) => {
        if (doctorId) {
            const token = getJwtToken();
            axios.get(`${API_BASE_URL}/doctor-service/api/doctorAvailability/getSchedule/${doctorId}`, { headers: { Authorization: `Bearer ${token}` } })
                .then(response => {
    ...
    const handleBookAppointmentSubmission = async () => {
        if (selectedSpecializationId && selectedDoctorId && appointmentDate && selectedSession) {
            ...
            try {
                ...
                const response = await axios.post('${API_BASE_URL}/login-service/api/patients/bookAppointment', bookingData, {
