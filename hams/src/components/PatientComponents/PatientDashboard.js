...
const PatientDashboard = () => {
    ...
    if (patientId && token) {
      try {
        const response = await axios.get(
          `${API_BASE_URL}/login-service/api/patients/${patientId}`,
          {
            headers: {
  ...
  const fetchAppointments = async () => {
    ...
    if (patientId && token) {
      try {
        const response = await axios.get(
          `${API_BASE_URL}/login-service/api/patients/current/patient/${patientId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
        ...
        <div className="dashboard-grid">
          ...
          <div className="appointments-card">
            ...
            <div class="welcome-message">
              Welcome back <strong>{patientInfo.name}</strong>!!!
              <p>
                You can now view your latest health information and manage your appointments here.
              </p>
            </div>
            {getNextUpcomingAppointment() && (
              <div className="next-appointment">
                ...
                <p>Doctor: {getNextUpcomingAppointment().doctorName}</p>
                <div className="next-appointment">
