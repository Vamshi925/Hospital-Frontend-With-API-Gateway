...
function Schedule() {
  ...
  const fetchSchedule = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/doctor-service/api/doctorAvailability/getSchedule/${doctorId}`,
        {
  ...
  const fetchAppointments = async () => {
    try {
      const token = localStorage.getItem('jwtToken');
      const response = await axios.get(
        `${API_BASE_URL}/doctor-service/api/doctors/current/appointments/${doctorId}`,
        {
          headers: {
  ...
  const handleToggleAvailability = async (availabilityId, date, session) => {
    try {
      const token = localStorage.getItem('jwtToken');
      await axios.put(
        `${API_BASE_URL}/doctor-service/api/doctorAvailability/toggle/${availabilityId}`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      setCellMessages((prev) => ({
        ...prev,
        [`${date}-${session}`]:
          prev[`${date}-${session}`] === 'Marked as unavailable' ? session : session,
      }));
    } catch (error) {
  ...
  const handleCellClick = (entry, session) => {
    if (entry[session] === 'booked') {
      const appointment = appointments.find(
        (app) => app.appointmentDate === entry.date && app.session === session
      );
      setPopupDetails({
        date: entry.date,
        session,
  ...
  const handleFilter = async () => {
    try {
      const token = localStorage.getItem('jwtToken');
      const response = await axios.get(
        `${API_BASE_URL}/doctor-service/api/doctorAvailability/filterSchedule/${startDate}/${endDate}/${doctorId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const groupedSchedule = schedule.reduce((acc, entry) => {
    ...
    if (!acc[date]) {
      acc[date] = {
        ...
        FN: 'available',
        AN: 'available',
        availabilityIdFN: null,
        availabilityIdAN: null,
      };
    }
    if (entry.session === 'FN') {
      acc[date].FN = toggledSlots.has(entry.availabilityId)
        ? 'not-available'
        : entry.isAvailable === 1
          ? 'available'
          : entry.isAvailable === 0
      ...
      acc[date].AN = toggledSlots.has(entry.availabilityId)
        ? 'not-available'
        : entry.isAvailable === 1
          ? 'available'
          : entry.isAvailable === 0
  ...
  return (
    <div className="schedule-page-container">
      {' '}
      {/* New container to manage overall layout */}
      <Header />
      <NavigationBar />
      <div className="schedule-content">
        {' '}
        {/* Container for the main schedule content */}
