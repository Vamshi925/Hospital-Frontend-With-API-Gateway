...
function DoctorAppointmentsTable() {
    ...
    const handleCancelConfirm = async () => {
        if (appointmentToCancel) {
            try {
                const token = localStorage.getItem('jwtToken');
                await axios.post(`${API_BASE_URL}/doctor-service/api/doctors/cancel/${appointmentToCancel}`, {}, {
                    headers: { Authorization: `Bearer ${token}` },
                });
    ...
    const handleFilter = async () => {
        try {
            const token = localStorage.getItem('jwtToken');
            const response = await axios.get(`${API_BASE_URL}/doctor-service/api/doctors/filterAppointmentsByDate/${startDate}/${endDate}/${doctorId}/${appointmentStatus}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            const sortedAppointments = response.data.data.sort((a, b) => new Date(a.appointmentDate) - new Date(b.appointmentDate));
            setAppointments(sortedAppointments);
