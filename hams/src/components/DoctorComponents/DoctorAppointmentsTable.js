...
function DoctorAppointmentsTable() {
  ...
  const handleCancelConfirm = async () => {
    if (appointmentToCancel) {
      try {
        const token = localStorage.getItem('jwtToken');
        await axios.post(
          `${API_BASE_URL}/doctor-service/api/doctors/cancel/${appointmentToCancel}`,
          {},
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        ...
        setAppointments(
          appointments.filter((appointment) => appointment.appointmentId !== appointmentToCancel)
        );
  ...
  const handleFilter = async () => {
    try {
      const token = localStorage.getItem('jwtToken');
      const response = await axios.get(
        `${API_BASE_URL}/doctor-service/api/doctors/filterAppointmentsByDate/${startDate}/${endDate}/${doctorId}/${appointmentStatus}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
        ...
        <div className="appointments-container">
          {appointments.map((appointment) => (
            <div key={appointment.appointmentId} className="appointment-card">
              ...
              <p>
                <strong>Session:</strong> {appointment.session}
              </p>
              <p>
                <strong>Status:</strong> {appointment.status}
              </p>
              <button
                className="view-medical-history-link"
