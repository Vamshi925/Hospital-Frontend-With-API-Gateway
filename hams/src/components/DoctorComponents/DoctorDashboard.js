...
const DoctorDashboard = () => {
  ...
  const fetchDoctorDetails = async () => {
    ...
    if (doctorId && token) {
      try {
        const response = await axios.get(`${API_BASE_URL}/doctor-service/api/doctors/${doctorId}`, {
          headers: {
        ...
        setErrorDoctorInfo(
          error.message || 'An unexpected error occurred while fetching doctor details'
        );
  ...
  const fetchAppointments = async () => {
    ...
    if (doctorId && token) {
      ...
      setErrorAppointments(
        'Doctor ID or Token not found. Please log in or ensure doctor information is available.'
      );
      setLoadingAppointments(false);
    }
  };
    ...
    const upcoming = appointments
      .filter(
        (appointment) =>
          ...
          isAfter(parseISO(appointment.appointmentDate), now)
      )
      .sort((a, b) => {
        const dateA = parseISO(a.appointmentDate);
        const dateB = parseISO(b.appointmentDate);
          ...
          <div className="appointments-card">
            ...
            {loadingDoctorInfo ? (
              ...
              <div class="welcome-message">
                Welcome back <strong>Dr. {name}</strong>!!!
                <p>You can now view your schedule and manage your appointments here.</p>
              </div>
            )}
            {getNextUpcomingAppointment() && (
              <div className="next-appointment">
                ...
                <p>
                  {new Date(getNextUpcomingAppointment().appointmentDate).toLocaleDateString()}{' '}
                  {getNextUpcomingAppointment().session}
                </p>
                <div className="next-appointment">
