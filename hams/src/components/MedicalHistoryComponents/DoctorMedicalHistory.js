  ...
  useEffect(() => {
    const fetchMedicalHistories = async () => {
      ...
      try {
        ...
        if (patientId) {
          if (startDate && endDate) {
            response = await axios.get(
              `${API_BASE_URL}/doctor-service/api/doctors/filterByDate/${startDate}/${endDate}/${patientId}`,
              headers
            );
          } else {
            response = await axios.get(
              `${API_BASE_URL}/doctor-service/api/doctors/medical-history/patient/${patientId}`,
              headers
            );
          }
        } else {
          response = await axios.get(
            `${API_BASE_URL}/doctor-service/api/doctors/medical-history/doctor/${doctorId}`,
            headers
          );
        }
        ...
        setError(
          'Failed to fetch medical history. Please check your network connection and try again.'
        );
