    ...
    if (patientId && token) {
      try {
        const response = await axios.get(
          `${API_BASE_URL}/login-service/api/patients/medicalHistory/${patientId}`,
          {
            headers: {
            ...
            {medicalHistory.map((record) => (
              <div key={record.historyId || Math.random()} className="history-record">
                ...
                <div className="record-details">
                  <p>
                    <strong>Date:</strong> {record.dateOfVisit}
                  </p>
                  <p>
                    <strong>Doctor Name:</strong> {record.doctorName}
                  </p>
                  <p>
                    <strong>Specialization:</strong> {record.specializationName}
                  </p>
                  <p>
                  ...
                  <p>
                    <strong>Medication:</strong> {record.medications}
                  </p>
                  <p>
                    <strong>BP:</strong> {record.bloodPressure}
                  </p>
                  <p>
